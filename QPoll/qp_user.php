...
class qp_Setup {
	...
	static $questionTypes = array(
		...
		'free' => array(
			'ctrl' => 'qp_FreeQuestion',
			'view' => 'qp_FreeQuestionView',
			'mType' => 'textQuestion'
		)
	);
	...
	static function init() {
		...
		self::autoLoad( array(
			...
			'ctrl/question/qp_freequestion.php' => array( 'qp_FreeQuestionOptions', 'qp_FreeQuestion' ),
			...
			'view/question/qp_freequestionview.php' => 'qp_FreeQuestionView',
	...
	static function getCurrUserName() {
		global $wgRequest, $wgUser, $wgSquidServers;
		...
		if ( self::$anon_forwarded_for === true && $wgUser->isAnon() ) {
			...
			$forwardedFor = function_exists( 'wfGetForwardedFor' ) ?
				wfGetForwardedFor() :
	 ...
	 *   raw attributes string, for example: 'a="b" c=1'
	 * @param    $attr_list array
	 *   list of XML attribute names (PCRE match)
	 * @modifies $attr_vals  array
	 *   key is attribute regexp
	 *   value is the value of attribute or null
	 * @return  int
	 *   count of found attributes
	 */
	static function getXmlLikeAttributes( $attr_str, array $attr_list, array &$attr_vals ) {
		# performance optimization: make sure $attr_vals was properly initialized in caller.
		# $attr_vals = array();
		$match = array();
		$count = 0;
		foreach ( $attr_list as $attr_name ) {
			...
			if ( count( $match ) > 1 ) {
				...
				$attr_vals[$attr_name] = array_pop( $match );
				$count++;
			} else {
				$attr_vals[$attr_name] = null;
			}
		}
		return $count;
	}
	...
	static function showPoll( $input, $argv, $parser, $frame = false ) {
		if ( !self::$request instanceof WebRequest ) {
			return '';
		}
		if ( !self::$cache_control ) {
			$parser->disableCache();
