...
/**
 * Stores category type, optional display attributes and the list of current category options.
 * Raw source contains pipe-separated entries enclosed into specified brackets.
 */
class qp_TextQuestionOptions {
	...
	# list of attributes which values has to be extracted from text input categories
	protected $textOptionAttributes = array( 'width', 'height', 'sorting', 'multiple' );
	# list of attributes which values has to be extracted from switch-like categories
	protected $switchOptionAttributes = array( 'checked' );
	 ...
	 * Optionally parses xml-like attributes (when these are found in category definition).
	 * Attribute 'name' is used only for question type="free".
	 */
	function addToLastOption( $token ) {
		...
		if ( $this->type === 'text' ) {
			...
			if ( count( $this->input_options ) === 1 &&
				...
				if ( qp_Setup::getXmlLikeAttributes( $matches[1], $this->textOptionAttributes, $option_attributes ) > 0 ) {
					...
					# there are no xml attributes, the whole match is new proposal name
			...
			if ( $token !== '' ) {
				# "switch" types of categories do not contain text values,
				# only xml-like attributes
				$option_attributes = array();
				if ( qp_Setup::getXmlLikeAttributes( $token, $this->switchOptionAttributes, $option_attributes ) > 0 ) {
					# apply attributes to current option
					if ( array_key_exists( 'name', $this->attributes ) ) {
						$this->attributes['name'] = $option_attributes['name'];
					}
					if ( $this->type === 'checkbox' ) {
						# checked is currently supported only for checkbox;
						# todo: support for radiobuttons as well? (requires backtrack of proposal view).
						$this->attributes['checked'] = $option_attributes['checked'];
					}
				} elseif ( array_key_exists( 'name', $this->attributes ) ) {
					# there are no xml attributes, the whole token value is new proposal name
					$this->attributes['name'] = trim( $token );
				}
			}
		}
...
class qp_TextQuestion extends qp_StubQuestion {
	 ...
	 * Also, stores checkbox / radio / text user answer into the parsed tokens list.
	 */
	function loadProposalCategory( $proposalId, $catId, $emptytext ) {
		global $wgContLang;
		...
		if ( ( $prev_text_answer = $this->answerExists( $this->opt->type, $proposalId, $catId ) ) !== false ) {
			$answered = true;
		...
		if ( $answered !== false ) {
			...
			if ( $this->opt->type !== 'text' ) {
				$this->opt->attributes['checked'] = true;
			}
		}
		$this->setCategory( $name, $text_answer, $answered );
	}

	/**
	 * Set category name and user answer for current category.
	 * @param  $name  string
	 *   html form input name
	 * @param  $text_answer  string
	 *   text answer (empty string when unavailable - checkbox, radiobutton, non-mandatory text field)
	 * @param  $answered  boolean
	 *   true, when current user already answered to proposal category, false otherwise
	 */
	 ...
	 */
	protected function findMatchingBraces() {
		$brace_stack = array();
	 ...
	 */
	protected function backtrackMismatchingBraces() {
	...
	/**
	 * Builds proposal view and dbtokens for the current proposal.
	 * @param  $proposalId  int
	 *   id of current proposal
	 * @return  int
	 *   count of categories defined for current proposal.
	 */
			...
			if ( $token === '|' ) {
				...
				if ( array_key_exists( 'closed_at', $brace_match ) &&
					...
					if ( array_key_exists( 'iscat', $brace_match ) ) {
						...
						$this->dbtokens[++$dbtokens_idx] = $this->opt->input_options;
						# setup mCategories
						$this->mCategories[$catId] = array( 'name' => strval( $catId ) );
						# load proposal/category answer (when available)
						$this->loadProposalCategory( $proposalId, $catId, qp_Setup::$propAttrs->emptytext );
	...
	function parseBody() {
		$proposalId = 0;
		$this->opt = new qp_TextQuestionOptions();
		...
		while ( $prop_attrs->iterate() ) {
			...
			$lastCatId = $this->parseTokens( $proposalId );
